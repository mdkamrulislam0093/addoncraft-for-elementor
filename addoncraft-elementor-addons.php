 ...
 * Text Domain: addoncraft-elementor-addons
 */
	...
	public function build_dependencies_notice() {
        ...
        if ( ! did_action( 'elementor/loaded' ) && is_admin() ) {
	        ...
	        if ($this->is_plugin_installed($elementor)) {
	            ...
	            $message = sprintf(__('%1$sAddonCraft for Elementor%2$s requires %1$sElementor%2$s plugin to be active. Please activate Elementor to continue.', 'addoncraft-elementor-addons'), "<strong>", "</strong>");

	            $button_text = __('Activate Elementor', 'addoncraft-elementor-addons');
	        } else {
	            $activation_url = wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=elementor'), 'install-plugin_elementor');

	            $message = sprintf(__('%1$sAddonCraft for Elementor%2$s requires %1$sElementor%2$s plugin to be installed and activated. Please install Elementor to continue.', 'addoncraft-elementor-addons'), '<strong>', '</strong>');
	            $button_text = __('Install Elementor', 'addoncraft-elementor-addons');
	        }
